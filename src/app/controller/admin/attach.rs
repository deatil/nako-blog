...
pub async fn index(
    ...
    Ok(nako_http::view(view, "admin/attach/index.html", &ctx))
}
...
pub async fn detail(
    ...
    if data.id == 0 {
        return Ok(nako_http::error_response_html(&view, "附件不存在", ""));
    }

    let mut ctx = nako_http::view_data();
    ctx.insert("data", &data);

    Ok(nako_http::view(view, "admin/attach/detail.html", &ctx))
}
