...
pub async fn index(
    ...
    let (arts, num_pages) = art::ArtModel::list_in_page(db, page, per_page, search_where.clone()).await.unwrap_or_default();
    let _count = art::ArtModel::list_count(db, search_where.clone()).await.unwrap_or(0);
    ...
    Ok(nako_http::view(&mut view, app::view_path("list.html").as_str(), &ctx))
}
...
pub async fn name(
    ...
    Ok(nako_http::view(&mut view, app::view_path(cate_data.list_tpl.as_str()).as_str(), &ctx))
}
